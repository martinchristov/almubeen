...
const Lines = ({ p, setSelectedAya, markAya, setIframe }) => {
  ...
  lines.map((line, lineIndex) => {
    ...
    if (line[0].verseNumber === 1 && p > 2) {
      if (!(lineIndex > 0 && lines[lineIndex - 1][0].verseNumber === 1)) {
        // additional check for cases when the first verse may be on multiple lines
        ...
        if (p !== 187) {
          ret.push(
            <span key="bismillah" className="bismillah">
    ...
    const lineContent = line.map((word) => {
      let kalima = (
        <span
          className={`kalima l-${word.lineNumber} id-${word.id} t-${word.charTypeName}`}
          key={word.id}
        >
          {word.codeV2}
        </span>
        ...
        kalima = (
          <span
            ...
            className={classNames('marker', {
              marked: word.verseKey === markAya,
              bookmarked:
                collections.length > 0 &&
                collections[0].keys.indexOf(word.verseKey) !== -1,
            })}
    ...
    if (lineIndex === lines.length - 1) {
      ...
      if (
        line[line.length - 1].verseNumber ===
        surat.chapters[surahIndex].verses_count
      ) {
...
const PopupContent = ({ word, setIframe }) => {
  ...
  morphs.forEach((part, pi) => {
    morphWord.push(
      <>
        <b className={`m-${part[1]}`}>{`${removeUnknownChars(
          word.textUthmani.slice(ind, ind + part[0].length)
        )}${pi < morphs.length - 1 ? String.fromCharCode(8205) : ''}`}</b>
      </>
    )
    ind += part[0].length
  })
...
const Safha = ({
  ...
  return (
    <div
      className={classNames('page', `page${p}`, { loaded })}
      style={init ? { fontFamily: `page${p}` } : null}
    >
      <div className="sticky-page">
        <div>{ConvertToArabicNumbers(p)}</div>
      </div>

...
const getTranslation = (translation, chapter, verse) => {
  const ret = translation.quran.find(
    (it) => it.chapter === Number(chapter) && it.verse === Number(verse)
  )
  return ret?.text
}
...
const AyaModal = ({ selectedAya, setSelectedAya, page, setLoginModalOpen }) => {
  return (
    ...
    >
      <ModalContent
        {...{ selectedAya, setSelectedAya, page, setLoginModalOpen }}
      />
    </Modal>
  )
}
...
const ModalContent = ({
  ...
  const _selected = localStorage.getItem('selected-translations')
    ? JSON.parse(localStorage.getItem('selected-translations'))
    : []
        ...
        <>
          <div className="aya-title">
            ...
            <Button
              disabled={
                Number(keys[1]) >
                surat.chapters[Number(keys[0]) - 1].verses_count - 1
              }
              className="next"
              type="link"
              size="large"
          ...
          <div className="aya-arabic">
            {selectedAya &&
              pageData[page - 1]
                ?.find((it) => it.verseKey === selectedAya)
                ?.words?.map((it) => (
                  <span key={it.id}>{removeUnknownChars(it.textUthmani)} </span>
                ))}
          </div>
          {session != null && (
              ...
              <ul>
                {collections.map((it) => (
                  <li key={it.id}>
                    <Checkbox
                      value={it.id}
                      checked={it.keys.indexOf(selectedAya) !== -1}
                      onChange={handleCheck(it, keys)}
                    >
                      {it.title}
                    </Checkbox>
                  </li>
...
const EditSources = ({ handleChangeSources }) => {
  const [selected, setSelected] = useState(
    localStorage.getItem('selected-translations')
      ? JSON.parse(localStorage.getItem('selected-translations'))
      : []
  )
  let sourcesByLang = {}
  ...
  const handleChangeCheck = (source) => (e) => {
    setSelected((val) => {
      ...
      if (val.indexOf(source.key) !== -1)
        return val.filter((it) => it !== source.key)
      return val
    })
  }
  return (
          ...
          <CollapsePanel header={lang} key={lang}>
            <ul>
              {sourcesByLang[lang].map((source) => (
                <li key={source.key}>
                  <Checkbox
                    checked={selected.indexOf(source.key) !== -1}
                    onChange={handleChangeCheck(source)}
                  >
                    {source.author}
                  </Checkbox>
                </li>
